...
import { ArrowLeft, Clock, History, CheckCircle, AlertTriangle, Wifi, Trophy } from "lucide-react";
...
export default function SellerAuctionDetail() {
  ...
  const handleAcceptBid = (bidderId: number, amount: number) => {
    acceptWinnerMutation.mutate({ id, data: { winnerId: bidderId } }, {
      onSuccess: () => {
        toast.success(`Winner accepted at $${amount.toLocaleString()} — item moved to Sold Items`);
        queryClient.invalidateQueries({ queryKey: getGetAuctionQueryKey(id) });
        queryClient.invalidateQueries({ queryKey: getListSellerAuctionsQueryKey() });
        queryClient.invalidateQueries({ queryKey: getGetSellerStatsQueryKey() });
      },
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            {isSold && <Badge className="bg-green-500/20 text-green-500 hover:bg-green-500/30">SOLD</Badge>}
          </div>
          <h1 className="text-3xl font-black tracking-tight text-white uppercase">{auction.title}</h1>
        </div>
        <div className="flex gap-3">
          {isSold && (
            <RouterLink href="/seller/sold-items" className="contents">
              <Button variant="outline" className="border-green-500/50 text-green-500 hover:bg-green-500/10 font-bold uppercase tracking-wider text-xs gap-2">
                <Trophy className="w-4 h-4" /> View Sold Items
              </Button>
            </RouterLink>
          )}
          ...
          <CardContent className="p-6 flex flex-col items-center text-center gap-6">
            ...
            <div className="w-full bg-background/50 rounded-lg p-4 border border-border/50">
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">
                {isSold ? "Final Price" : "Current Ask"}
              </p>
              <p className={`text-4xl font-mono font-bold drop-shadow-[0_0_5px_rgba(220,38,38,0.5)] ${isSold ? "text-green-400" : "text-primary"}`}>
            ...
            {isSold && auction.winnerName && (
              <div className="w-full bg-green-500/10 border border-green-500/30 rounded-lg p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-bold text-green-500 uppercase tracking-wider">Sale Finalized</p>
                </div>
                <p className="text-lg font-bold text-white">{auction.winnerName}</p>
                <p className="text-xs text-muted-foreground mt-1">Sold for ${auction.currentBid.toLocaleString()}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="md:col-span-2 glass-card border-border/50 h-[560px] flex flex-col">
          <div className="p-4 border-b border-border/50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {isSold && (
                <Badge className="bg-green-500/20 text-green-500 border-green-500/30 text-xs">CLOSED</Badge>
              )}
            </div>
            ...
            {loadingBids ? (
              ...
              <div className="space-y-3">
                <AnimatePresence>
                  {bids.map((bid, i) => {
                    const isTopBid = i === 0;
                    const canAccept = !isSold && (requiresAction || (auction.status === "live" && isTopBid));
                    return (
                      <motion.div
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {auction.winnerId === bid.bidderId && (
                              <Badge className="bg-green-500/20 text-green-500 hover:bg-green-500/30 text-[10px]">WINNER</Badge>
                            )}
                          </div>
