...
import { ArrowLeft, Clock, History, CheckCircle, AlertTriangle } from "lucide-react";
...
export default function SellerAuctionDetail() {
  ...
  const { data: bids, isLoading: loadingBids } = useListBids(id, {
    query: { enabled: !!id, queryKey: getListBidsQueryKey(id), refetchInterval: auction?.status === "live" ? 3000 : false }
  });
  ...
  const handleEndEarly = () => {
    updateMutation.mutate({ id, data: { status: "ended", endTime: new Date().toISOString() } }, {
      ...
      onError: (err) => toast.error(err.data?.error || "Failed to end auction")
    });
  };
  ...
  const handleAcceptBid = (bidderId: number, amount: number) => {
    acceptWinnerMutation.mutate({ id, data: { winnerId: bidderId } }, {
      ...
      onError: (err) => toast.error(err.data?.error || "Failed to accept winner")
    });
  };

  if (loadingAuction) return <div className="p-8"><Skeleton className="h-96 w-full rounded-xl" /></div>;
  if (!auction) return <div className="text-center py-24"><h2>NOT FOUND</h2></div>;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            {auction.status === "live" && !isEnded && <Badge className="bg-primary animate-pulse shadow-[0_0_8px_rgba(220,38,38,0.8)]">LIVE</Badge>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1 bg-card border-border/50">
          <CardContent className="p-6 flex flex-col items-center text-center gap-6">
            <div className="w-full bg-background rounded-lg p-4 border border-border/50">
            ...
            <div className="w-full bg-background rounded-lg p-4 border border-border/50 flex flex-col items-center justify-center">
              <div className="flex items-center gap-2 text-white font-bold mb-1">
                <Clock className="w-4 h-4" /> <span className="uppercase text-xs tracking-wider">Clock</span>
              </div>
              {isEnded ? (
        ...
        <Card className="md:col-span-2 bg-card border-border/50 h-[500px] flex flex-col">
          <div className="p-4 border-b border-border/50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <History className="w-4 h-4 text-muted-foreground" />
              <h3 className="font-bold text-sm uppercase tracking-wider">Bid Ledger</h3>
            </div>
            <span className="text-xs font-mono text-muted-foreground">{bids?.length || 0} Total Bids</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {loadingBids ? (
               ...
               <div className="h-full flex items-center justify-center text-muted-foreground text-sm uppercase tracking-widest">No bids logged</div>
            ) : (
                   ...
                   {bids.map((bid, i) => {
                     ...
                     return (
                       <motion.div 
                         ...
                         key={bid.id} 
                         className={`p-4 rounded-lg border flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isTopBid ? 'bg-primary/5 border-primary/30' : 'bg-background border-border/50'}`}
                       >
                         <div>
                           ...
                           <p className="text-xs font-mono text-muted-foreground">{new Date(bid.createdAt).toLocaleString()}</p>
                         </div>
                         
                         <div className="flex items-center justify-between sm:justify-end gap-6">
                           <div className={`text-xl font-mono font-bold ${isTopBid ? 'text-primary' : 'text-white'}`}>
                           ...
                           {canAccept && auction.status !== "sold" && (
                             <Button 
                               size="sm" 
                               variant="outline"
                               className="font-bold tracking-widest text-xs h-8 border-primary/50 text-white hover:bg-primary"
